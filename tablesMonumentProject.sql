...
-- table Lieu
create table Lieu(codeInsee varchar(5) primary key, nomCom varchar(16), 
                    longitude float, latitude float, depart varchar(4));
...
-- and nomCom = 'MONTPELLIER';


-- table Departement
create table Departement(dep varchar(4) primary key, nomDep varchar(30), chefLieu varchar(5), reg varchar(4));

insert into Departement values('34', 'HERAULT', '34172', '76');
insert into Departement values('30', 'GARD', '30189', '76');


alter table Lieu add constraint Lieu_depart_fk foreign key (depart) references Departement(dep) on delete cascade;
alter table Departement add constraint Departement_chefLieu_fk foreign key (chefLieu) references Lieu(codeInsee) on delete cascade;

-- table monument
create table monument (geohash varchar(12) primary key, nom varchar(80), proprietaire varchar(10), 
                        typeM varchar(20), longitude float, latitude float, codeInsee varchar(5));
...
-- table Celebrite
create table Celebrite(numCelebrite integer primary key, nom varchar(16), prenom varchar(16), nationalite varchar(10), epoque varchar(6));
...
-- AssocieA
create table AssocieA(codeM varchar(12), numCelebrite integer, constraint AssocieA_pk primary key (codeM,numCelebrite));
...
alter table monument add constraint monumentcode foreign key (codeInsee) references Lieu(codeInsee) on delete cascade;

alter table AssocieA add constraint AssocieA_codeM_fk foreign key (codeM) references Monument(geohash) on delete cascade;
alter table AssocieA add constraint AssocieA_numCelebrite_fk foreign key (numCelebrite) references Celebrite(numCelebrite) on delete cascade;
